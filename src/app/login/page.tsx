      ...
      <div className="glass w-full max-w-md rounded-2xl p-6 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3.5 text-sm text-white placeholder-gray-500 outline-none focus:border-[#00FF88]/50 focus:ring-1 focus:ring-[#00FF88]/50" placeholder="tu@ejemplo.com" />
          </div>
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-300">Contraseña</label>
            <div className="relative">
              <input type={showPassword ? "text" : "password"} required value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3.5 pr-12 text-sm text-white placeholder-gray-500 outline-none focus:border-[#00FF88]/50 focus:ring-1 focus:ring-[#00FF88]/50" placeholder="••••••••" />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300">
                {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
              </button>
            </div>
          </div>
          <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#00FF88] py-3.5 text-sm font-semibold text-black transition-all hover:bg-[#00CC6A] disabled:opacity-50">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-3 rounded-xl bg-[#5865F2] py-3.5 text-sm font-semibold text-white transition-all hover:bg-[#4752c4]"
        >
